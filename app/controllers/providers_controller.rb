class ProvidersController < ApplicationController
  ...
  def harvest_all_selective_intermediate_provider
    queue_name = "harvest"
    workers = "workers"
    Resque.enqueue(HarvestAllByIntermediateProvider, @provider)
    redirect_to providers_url, notice: "All seeds from \"#{@provider.intermediate_provider}\" being harvested in the background.  Currently at position ##{Resque.size(queue_name) + Resque.working.size} in the queue. #{Resque.working.size} #{workers.pluralize(Resque.working.size)} currently working on a task."
  end
