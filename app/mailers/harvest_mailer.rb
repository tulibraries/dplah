class HarvestMailer < ActionMailer::Base
  ...
  def dumped_whole_index_email
    subject_text = "Whole Index Deleted"
    m = mail(subject: subject_text).deliver
  end

  def dump_and_reindex_by_institution_email(provider)
    @provider = provider
    subject_text = "Dumped and reindexed #{provider.name} collections"
    m = mail(to: provider.email, subject: subject_text).deliver
  end

  def dump_and_reindex_by_collection_email(provider)
    @provider = provider
