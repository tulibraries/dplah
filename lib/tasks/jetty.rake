  ...
  namespace :dplah do
    namespace :jetty do
      ...
      task :config => ['jetty:stop', 'jetty:clean'] do
        ...
        Rake::Task["dplah:jetty:config_solr"].reenable
        Rake::Task["dplah:jetty:config_solr"].invoke
      end
  
      desc "Copies the default SOLR config for the bundled Dplah Testing Server"
      task :config_solr do
      ...
      desc "Copies a custom fedora config for the bundled Dplah Testing Server"
